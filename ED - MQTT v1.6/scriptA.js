 ...
 * License: MIT
 * ----------------------------------------------------------
 * edited heavely to fit this projects goals by: Jesper Jensen, Mia Hartmann, Rasmus Jespersen and Rasmus Dhen
 */
...
window.onload = (event) => {
    ...
    function RS(target, event, vertical) {
        ...
        function dragUpdate(e) {
            ...
            var pos = e.touches ? e.touches[0][page] : e[page],
                ...
                value = edge(((pos - rangerDistance) / rangerSize) * 100, 0, 100); // brug denne value
            ...
            if (drag) {
                ...
                // color changer
                dragger.addEventListener("mousemove", function () {
                    var color = 'linear-gradient(180deg, rgb(0,0,0)' + value + '%,rgb(255,255,255)' + value + '%)';
                    ranger.style.background = color;
                })

                dragger.addEventListener("touchmove", function () {
                    var color = 'linear-gradient(180deg, rgb(0,0,0)' + value + '%,rgb(255,255,255)' + value + '%)';
                    ranger.style.background = color;
                })
    ...
    // output - brug denne til MQTT
    RS(document.getElementById('range-slider-2'), {
        value: 50, // initial value
        vertical: true,
        drag: function (v) {
            //testResult[1].innerHTML = v; // printer nummerisk værdi af slideren på siden
            sendMQTT(v)  // sender sliderens værdi
        }
    });
...
/*function lightmessage(){  
    //console.log(v + " inside RS");
    sendMQTT(window.lightV);
    } 
